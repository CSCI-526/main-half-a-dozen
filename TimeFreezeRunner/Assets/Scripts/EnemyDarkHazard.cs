...
{
    [Header("Colors")]
    public Color activeColor = Color.red;                         // when game is playing
    public Color frozenColor = new Color(0.6f, 0.1f, 0.1f, 0.7f); // before game starts
    ...
    void Awake()
    {
        ...
        // ensure tintable material
        var mat = Shader.Find("Sprites/Default");
        if (mat != null) sr.material = new Material(mat);

        sr.sortingOrder = 30;
        sr.color = frozenColor * new Color(1,1,1,0); // start hidden

        col.isTrigger = true;
        rb.bodyType   = RigidbodyType2D.Kinematic;
        rb.simulated  = true;

        // sanity: avoid bad radius setup
    ...
    {
        if (!armed) return;                  // only lethal when game running
        if (!other.CompareTag("Player")) return;
        GameManager.I?.OnPlayerCaught();     // your existing lose flow
    }

    // If someone unchecks IsTrigger and wants solid colliders, we still catch:
